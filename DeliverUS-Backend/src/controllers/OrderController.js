...
const _sortOrdersByStatus = (orders) => {
  const statusOrder = {
    pending: 0,
    'in process': 1,
    sent: 2,
  ...
  return orders.sort((a, b) => {
    const statusDiff = statusOrder[a.getStatus()] - statusOrder[b.getStatus()]
    if (statusDiff !== 0) {
      return statusDiff
    }
    // Mismo estado: ordenar por fecha de creación ascendente
...
const indexRestaurant = async function (req, res) {
  ...
  try {
    ...
    orders = _sortOrdersByStatus(orders)
...
const analytics = async function (req, res) {
  ...
  try {
    ...
    const invoicedToday = await Order.sum(
      ...
      }) || 0 // Si invoicedToday es null, lo reemplazamos por 0
...
const updateByOwner = async function (req, res) {
  try {
